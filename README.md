...
[![Version](https://img.shields.io/badge/version-0.3.13-1769c2)](#windows-桌面端)
...
- 成果交付：Agent 和工作流产出、运行轨迹及文档元数据写入 SQLite；研究任务可生成 `.md` 文件，并以排版后的 Markdown、表格、代码和 KaTeX 公式在独立文档区域渲染。
...
- 智能编排专家：根据自然语言生成工作流、分支、变量、知识库节点以及配置完整的新 Agent；用户可持续对话调整方案，生成后仍可手工修改节点、连线和参数。
- 工作流运行：实时显示每个节点及 Agent 内部 RAG、联网、模型、工具和成果事件；支持循环次数、停止条件、自动重试、运行中暂停、中断、人工引导和每轮成果文档。
...
```text
frontend/src-tauri/target/release/bundle/nsis/EvoAgent_0.3.13_x64-setup.exe
frontend/src-tauri/target/release/bundle/msi/EvoAgent_0.3.13_x64_en-US.msi
```
...
当前桌面版本为 **v0.3.13**。源码构建后，主程序和安装包位于 `frontend/src-tauri/target/release/`。正式版将用户数据独立保存在 `%LOCALAPPDATA%\EvoAgent\`，升级或替换程序不会覆盖数据库、密钥、工作区、Skills 和插件。
