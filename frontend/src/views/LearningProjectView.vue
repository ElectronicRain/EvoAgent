...
const pathGoalDraft = ref('')
const pathTargetLevelDraft = ref('proficient')
...
async function load() {
  loading.value = true
  try {
    // Workspace is loaded first because it performs the one-time migration
    // from broad curriculum nodes to goal-specific micro knowledge points.
    const workspace = await api.get<Entity>(`/learning-projects/${projectId.value}/workspace`)
    const [subjectPack, diagnosis, path, space] = await Promise.all([
    ...
    pathGoalDraft.value = workspace.project?.target || ''
    pathTargetLevelDraft.value = workspace.project?.target_level || 'proficient'
...
async function refreshAdaptivePath() {
  const [diagnosis, path] = await Promise.all([
    api.get<Entity>(`/learning-projects/${projectId.value}/diagnostic`),
    api.get<Entity>(`/learning-projects/${projectId.value}/personalized-path`),
  ])
  diagnostic.value = diagnosis
...
async function applyPathGoal() {
  const goal = pathGoalDraft.value.trim()
  if (goal.length < 4 || busy.value) return store.notify('请填写具体、可验证的当前学习目标', 'error')
  busy.value = 'path-goal'
  try {
    await api.patch(`/learning-projects/${projectId.value}`, { target: goal, target_level: pathTargetLevelDraft.value })
    await api.post(`/learning-projects/${projectId.value}/direction/regenerate`, { keep_memories: true })
    selectedNodeId.value = ''
    await load()
    store.notify('已按当前目标重建原子知识点路径，并保留学习记忆与可迁移掌握证据')
  } catch (error: any) { store.notify(error.message || '目标路径重建失败', 'error') }
  finally { busy.value = '' }
}
...
async function updateTask(task: Entity, status: string) {
  try { const updated = await api.patch<Entity>(`/learning-projects/${projectId.value}/tasks/${task.id}`, { status }); Object.assign(task, updated); data.value.project.progress = Math.round(100 * data.value.tasks.filter((item: Entity) => item.status === 'completed').length / Math.max(1, data.value.tasks.length)); await refreshAdaptivePath() }
...
watch(section, value => { if (value === 'path' && !loading.value) void refreshAdaptivePath() })
onMounted(load)
</script>
  ...
  <div v-else class="learning-project">
    ...
    <main class="section-content">
      ...
      <template v-else-if="section==='path'">
        <section class="panel full-panel"><header><div><h2>目标驱动的原子知识点路径</h2><p>不是课程大纲：每个节点都对应一个可讲解、可练习、可测量的小知识点，并随学习证据自动更新。</p></div><button class="btn btn-primary btn-sm" :disabled="busy==='path'" @click="replanPath"><RefreshCw :size="13" />{{ busy==='path'?'正在重排…':'按最新证据重排' }}</button></header><section class="path-goal-editor"><label><span>当前学习目标</span><textarea v-model="pathGoalDraft" class="textarea" placeholder="例如：能独立推导归并排序复杂度、实现代码并解释适用边界"/></label><label><span>目标学习深度</span><select v-model="pathTargetLevelDraft" class="select"><option value="foundation">理解概念</option><option value="intermediate">掌握机制与方法</option><option value="proficient">能够迁移应用</option><option value="advanced">能够综合优化与研究</option></select></label><button class="btn" :disabled="busy==='path-goal'" @click="applyPathGoal"><Target :size="13"/>{{ busy==='path-goal'?'正在重建…':'应用目标并重建路径' }}</button><aside><strong>当前开放深度 {{ personalizedPath.active_depth || 1 }}/{{ personalizedPath.target_depth || 1 }}</strong><span>{{ personalizedPath.adaptive_summary }}</span><small>下一达标证据：{{ personalizedPath.next_checkpoint }}</small></aside></section><div class="diagnostic-strip"><strong>{{ diagnostic.overall_score }}</strong><span><b>{{ diagnostic.level }}阶段</b><small>{{ diagnostic.limitations }}</small></span><article v-for="(value,key) in diagnostic.dimensions" :key="key"><label>{{ ({knowledge_mastery:'知识掌握',practice_accuracy:'练习正确率',task_progress:'任务进度',mistake_correction:'错题订正',learning_engagement:'学习投入'} as Record<string,string>)[String(key)]||key }}</label><b>{{ value }}%</b></article></div><LearningPathGraph :path="personalizedPath" @select="openPathNode"/><div class="path-actions"><article v-for="item in diagnostic.recommended_actions||[]" :key="item"><CheckCircle2 :size="14"/><span>{{ item }}</span></article></div></section>
      </template>
