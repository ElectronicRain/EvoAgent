...
import {
  ...
  Download, LogOut, MessageSquareText, Palette, RefreshCw, Save, Sparkles, UserRound,
  Cloud, ShieldCheck,
...
const telemetryStatus = ref<Entity>({ enabled:true, hub_configured:false, pending:0 })
  ...
  <div class="perception-page">
    ...
    <section class="bottom-grid">
      ...
      <aside class="account-panel panel-card">
        ...
        <p class="privacy-note"><Fingerprint :size="13" />账号内容、提问正文和用户画像保存在本地；系统只记录脱敏后的功能使用与错误事件。</p>
      </aside>
    </section>

    <section class="update-settings panel-card">
      <header class="card-heading"><div><span><ShieldCheck :size="15" />PRIVACY & DIAGNOSTICS</span><h3>隐私保护与运行诊断</h3><p>日志不包含密码、密钥、文档正文、对话正文或本地绝对路径。</p></div></header>
      <div class="telemetry-setting-body"><Cloud :size="22"/><div><strong>{{ telemetryStatus.hub_configured?'中央同步已配置':'本地离线队列' }}</strong><span>{{ telemetryStatus.hub_configured?'网络恢复后自动补传脱敏事件。':'尚未连接中央服务，事件只保存在当前设备。' }}</span></div><b>{{ telemetryStatus.pending||0 }} 条待同步</b></div>
    </section>

    <section class="update-settings panel-card">
      <header class="card-heading"><div><span><Download :size="15" />SOFTWARE UPDATE</span><h3>软件更新</h3><p>从官方发布源检查带签名的 Windows 更新包；发现更新后由你决定是否安装。</p></div><button class="btn btn-primary" :disabled="updater.checking||updater.downloading" @click="updater.check(true)"><RefreshCw :size="13"/>{{ updater.checking?'正在检查':'检查更新' }}</button></header>
      <div class="update-setting-body"><div><strong>EvoAgent V{{ updater.installedVersion }}</strong><span>当前安装版本</span><small v-if="updater.lastCheckedAt">上次检查：{{ formatDate(updater.lastCheckedAt) }}</small></div><div class="memory-setting"><div><RefreshCw :size="17"/><span><strong>启动时自动检查</strong><small>只检查版本和更新说明，不会未经确认自动安装。</small></span></div><button :class="{active:updater.automaticCheck}" @click="updater.setAutomaticCheck(!updater.automaticCheck)" :aria-label="updater.automaticCheck?'关闭自动检查':'开启自动检查'"><i /></button></div></div>
    </section>
  </div>
</template>
...
.update-settings{overflow:hidden}.update-setting-body{padding:14px 18px;display:grid;grid-template-columns:230px 1fr;align-items:center;gap:18px}.update-setting-body>div:first-child{display:grid;gap:3px}.update-setting-body>div:first-child strong{color:#214d6d;font-size:15px}.update-setting-body>div:first-child span,.update-setting-body>div:first-child small{color:#8194a2;font-size:8px}.update-setting-body .memory-setting{margin:0}
.telemetry-setting-body{padding:16px 18px;display:flex;align-items:center;gap:11px;color:#2279ad}.telemetry-setting-body>div{min-width:0;flex:1;display:grid;gap:3px}.telemetry-setting-body strong{color:#315a76;font-size:10px}.telemetry-setting-body span{color:#8295a3;font-size:8px}.telemetry-setting-body>b{color:#96651e;font-size:8px}
</style>
