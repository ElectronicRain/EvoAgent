...
import { CheckCircle2, Download, LoaderCircle, RefreshCw, ShieldCheck } from 'lucide-vue-next'
...
<template>
  <FloatingPanel v-model="model" title="软件更新" eyebrow="EVOAGENT UPDATE" description="更新包需通过 EvoAgent 发布签名验证，是否安装始终由你决定。" size="medium" :close-on-backdrop="!updater.downloading">
    ...
    <div v-else class="update-state"><CheckCircle2/><strong>{{ updater.error ? '暂时无法检查更新' : '当前已是最新版本' }}</strong><span>{{ updater.error || updater.sourceNote || `EvoAgent V${updater.installedVersion} 暂无可用更新。` }}</span></div>
    <template #footer>
