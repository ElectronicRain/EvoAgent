...
3. 将语义单元组装为父块。
4. 在每个父块内部生成带重叠的检索子块。
5. 子块保存父块 ID、页码/章节/幻灯片等定位元数据。
6. 对规范化后的子块计算 Hash，删除重复子块。
7. 只对检索子块生成向量；父块用于命中后的上下文扩展。
...
主页只显示四个必要阶段，避免将内部调试信息直接暴露给普通用户：

1. **查询改写**：生成的检索式数量；
2. **混合召回**：Dense、BM25 和融合候选数量；
3. **相关性重排**：重排候选数与最终保留片段数；
4. **证据生成**：上下文字符数和引用数量。
...
3. 创建知识库并导入初始数据。
4. 配置 Embedding、Rerank 和生成模型。
5. 选择全库、分组或当前知识库执行问答。
6. 查看四步精简检索过程、答案和引用片段。

### 7.2 独立管理窗口

点击知识库卡片后打开独立窗口，包含：

- **文档管理**：多文件上传、文字录入、文档选择与删除；
...
当前基线结果：后端 36 项测试通过，Ruff、Vue/TypeScript 生产构建和 Rust/Tauri 编译通过。
